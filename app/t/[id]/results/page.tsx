...
export default function TournamentResultsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (payload.summary_text.trim() && typeof window !== "undefined") {
  ...
  useEffect(() => {
    if (!selectedPlayer && filteredPlayers[0]) {
      setSelectedResultId(filteredPlayers[0].id);
      return;
    }

    if (selectedPlayer && selectedResultId !== selectedPlayer.id) {
      setSelectedResultId(selectedPlayer.id);
    }
  }, [filteredPlayers, selectedPlayer, selectedResultId]);
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-[linear-gradient(180deg,#f7f4ed_0%,#eef3f8_100%)] px-4 py-8 sm:px-6">
